...
def create_cache_key(url, params=None):
    ...
    if params:
        ...
        return urljoin(url, querystring).lower()
    else:
        return url.lower()
...
def get_resource(filepath, url, params=None, timeout=10):
    ...
    if key in get_cache(filepath).keys():
        return get_cache(filepath)[key]
    else:
        resource = get_swapi_resource(url, params, timeout)
        get_cache(filepath)[key] = resource
        return resource
...
def convert_data(person, planet_data):
    ...
    for key in person.keys():
        if person[key] in ('n/a', 'none', 'unknown'):
                person[key] = None
    for key, val in person.items():
        if key == 'homeworld':
            for planet in planet_data:
                if planet['name'] == get_swapi_resource(val)['name']:
                    person[key] = create_homeworld(planet)
        elif key == 'species':
            if person.get(key):
                person[key] = create_species(get_resource(CACHE_NAME, val[0]))
            else:
                person[key] = []
