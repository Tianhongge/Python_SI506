...
def get_passenger_info(passenger, characteristics):
    ...
    passenger_info = {}
    for key,val in passenger.items():
        if key in characteristics:
            passenger_info[key] = val
            if key =='homeworld':
                passenger_info[key] = get_swapi_resource(val)['name']
            elif key == 'species':
                passenger_info[key] = get_swapi_resource(val[0])['name']
...
def get_starships(person):
    ...
    for url in starships_urls:
        starship_dict = {}
        starship = get_swapi_resource(url)
        for key, val in starship.items():
            if key in ('name', 'max_atmosphering_speed', 'passengers'):
                starship_dict[key] = val
...
def get_fastest_starship(person):
    ...
    for i in range(len(starships)):
        if int(starships[i]['max_atmosphering_speed']) > max_speed and int(starships[i]['passengers']) >= 3:
            max_speed = int(starships[i]['max_atmosphering_speed'])
            i += 1
        else:
            i += 1
    for starship in starships:
        if int(starship['max_atmosphering_speed']) == max_speed:
            return {'name': starship['name'], 'max_atmosphering_speed': starship['max_atmosphering_speed'], 'passengers': starship['passengers']}
...
def board_ship(fastest_starship, passengers_list):
    ...
    order = []
    for passenger in enumerate(passengers_list, 1):
        (count, ele) = passenger
        passenger_reorder = (ele['name'], count)
        order.append(passenger_reorder)
    fastest_starship['boarding_order'] = order
...
def main():
    ...
    passengers_list = []
    for name in names:
        passenger = get_swapi_resource(base_url + '/people', {'search': name})['results'][0]
        passenger_info = get_passenger_info(passenger, characteristics)
    ...
    print(f"\nProblem 3: passengers_list = {passengers_list}")
