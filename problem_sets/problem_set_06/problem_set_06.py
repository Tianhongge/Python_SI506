...
def clean_row(game):
    ...
    for key, val in game.items():
        if key == 'ROUND':
            game[key] = rounds[val]
        elif key in ('WTEAM', 'LTEAM') and val in names.keys():
            game[key] = names[val]
...
def score_game_prediction(pred, games):
    ...
    for game in games:
        if game['ROUND'] == pred['ROUND'] and game['WTEAM'] == pred['WTEAM']:
            return int(points[game['ROUND']])
        else:
            continue
    return 0
...
def subtract_scores(game):
    ...
    Returns:
     ...
     """
    return abs(int(game['WSCORE'] - game['LSCORE']))
...
def main():
    ...
    underdogs = []
    for game in games_2021:
        if game['WSEED'] > game['LSEED']:
            underdogs.append(
                {
                game['WTEAM']: game['WSEED'],
                game['LTEAM']: game['LSEED'],
                }
            )
    print(f'\nProblem 3: underdogs =\n{underdogs}')
    ...
    total_points = 0
    for pred in dk_pred_2021:
        pred['BRACKETSCORE'] = score_game_prediction(pred, games_2021)
        total_points += pred['BRACKETSCORE']
    ...
    champ_count = {}
    for game in championships_1985_2021:
        if game['WTEAM'] not in champ_count.keys():
            champ_count[game['WTEAM']] = 1
        else:
            champ_count[game['WTEAM']] += 1
    ...
    max_diff = 0
    for game in championships_1985_2021:
        if subtract_scores(game) > max_diff:
            max_diff = subtract_scores(game)
            max_diff_game = game
