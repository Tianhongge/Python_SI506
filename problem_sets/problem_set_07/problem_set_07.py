...
def get_books_by_rating(books, rating, num_reviews=50000):
    ...
    for book in books:
        if book['rating'] > rating and book['num_reviews'] > num_reviews:
            books_by_rating.append(book['title'])
...
def update_books(books, details):
    ...
    for key in details.keys():
        for book in books:
            if details[key]['title'].lower() == book['title'].lower():
                book['title'] = details[key]['title']
                book['author'] = details[key]['author']
                book['ISBN_10'] = details[key]['ISBN_10']
                book['genre'] = details[key]['genre']
                book['price'] = details[key]['price']
                book['discount'] = details[key]['discount']
...
def get_book_price(books, title, format):
    ...
    """
    for book in books:
        price_keys = list(book['price'].keys())
        for i in range(len(price_keys)):
            price_keys[i] = price_keys[i].lower()
        if title.lower() == book['title'].lower() and format.lower() in price_keys:
            return {book['title']:book['price'][format.capitalize()]}
    else:
        return None
...
def get_books_by_genres(books, genres):
    ...
    for book in books:
        for genre in book['genre']:
            if genre in genres:
                if book['title'] not in books_by_genres.keys():
                    books_by_genres[book['title']] = book['author']

    return books_by_genres
