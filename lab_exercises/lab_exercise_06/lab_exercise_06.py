...
def get_team_most_points(teams):
    ...
    for team in teams:
        if team['Score'] > max_points:
            ...
            team_with_most_point = {}
            team_with_most_point['Name']=team['Team']
            team_with_most_point['Points']= team['Score']

    return team_with_most_point
...
def get_final_four(teams):
    ...
    for i in range(0, len(teams), 2):
        if teams[i]['Score'] > teams[i+1]['Score']:
            final_four[teams[i]['Team']] = teams[i]['Score']
        else:
            final_four[teams[i+1]['Team']] = teams[i+1]['Score']
    return final_four
...
def get_avg_fouls(teams):
    ...
    for team in teams:
        total_fouls += team['Fouls']

    return total_fouls // len(teams)
