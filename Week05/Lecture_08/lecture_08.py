...
charging_stations = data[i:] # TODO Access charging stations
um_count = 0 # TODO Initialize
i = 0 # TODO Initialize
while i in range(len(charging_stations)):
    if charging_stations[i][0].lower().startswith('U-M'):
        um_count += 1
    i += 1
# TODO Implement loop
...
i = 0 # TODO Intialize
while i < len(charging_stations):
    name = charging_stations[i][0]
    if 'ANN ARBOR' in name:
        charging_stations[i][0] = name.replace('ANN ARBOR', 'Ann Arbor')
    i += 1
# TODO Implement loop
