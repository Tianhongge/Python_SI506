...
def main():
    ...
    cwd = os.getcwd() # TODO Assign path

    print(f"\n1.0.1 Current working directory = {cwd}")

    # Absolute path to directory in which *.py is located.
    abs_path = os.path.dirname(os.path.abspath(__file__)) # TODO Assign path

    print(f"\n1.0.2 Absolute directory path = {abs_path}")

    # Construct macOS and Windows friendly paths
    # faculty_path = os.path.join(abs_path, 'umsi-faculty.txt') # TODO Assign path
    # resnick_path = os.path.join(abs_path, 'resnick-citations.csv') # TODO Assign path
    ...
    publications = read_csv(resnick_path) # TODO Call function

    print(f"\nCh 01 example publication\n{publications[1]}")

    # Get headers
    headers = publications[0] # TODO Get header row

    print(f"\nCh 01 Headers\n{headers}")

    # Test function
    total_citations = get_attribute(publication=publications[9], headers=headers, column_name='Total Citations') # TODO Call function
    ...
    max_citations = []
    max_count = 0 # TODO Assign sensible value
    for publication in publications[1:]:
        citation_count = int(get_attribute[publications, headers, 'Total Citations'])

        if citation_count > max_count:
            max_citations.clear()
            max_citations = citation_count
            max_citations.append(publication)
        elif citation_count == max_citations:
            max_citations.append(publication)
        else:
            continue
    ...
    min_citations = []
    min_count = float('inf') # TODO Assign sensible value
