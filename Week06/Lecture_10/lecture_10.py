...
wheaties_slogan = format_slogan(cereal, slogan)

print(f"\n1.3.1 {wheaties_slogan}")

x = 'Trix'
y = 'Silly Rabbit. Trix are for Kids.' # General Mills Trix
trix_slogan = format_slogan(x, y)
...
lucky_charms = format_slogan(slogan=slogan, name=cereal)
...
honey_nut_cheerios = format_slogan(cereal, slogan, separator='\n') # TODO call function (line break passed)
...
cinn_toast_crunch = format_slogan(cereal, slogan, ': ', all_caps=True) # TODO call function

print(f"\n1.6.3 {cinn_toast_crunch}")
...
post_cereals = get_cereals_by_company(cereal, 'post') # TODO call function
kellogg_cereals = get_cereals_by_company(cereal, 'KELLOGG') # TODO call function
