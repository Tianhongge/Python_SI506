...
def add_group(country, group):
    ...
    if country['country_code'] == group['country_code']:
        return  {
...
def main():
    ...
    print(f"\n1.0: for loop: country codes (n={len(country_codes)})\n = {country_codes[:5]}")

    # Clear list
    country_codes.clear() # redundant

    print(f"\n1.0: country codes cleared (n={len(country_codes)})")

    # List comprehension
    country_codes = [country['country_code'] for country in countries] # TODO list comp

    # Print slice (last 5 codes)
    print(f"\n1.0: list comp: country codes (n={len(country_codes)})\n = {country_codes[-5:]}")
    ...
    # Print slice
    # print(f"\n2.1.1 for loop: East Asia & Pacific (n={len(east_asia)})\n = {east_asia[:2]}")
    ...
    print(f"\n2.1.2: list comp: East Asia & Pacific (n={len(east_asia)})")
    ...
    americas_lower_middle = [
        country
        for country in countries
        if country['region'] in ('Latin America & Caribbean', 'North America')
        and country['income_group'] == 'Lower middle income'
    ]

    print(f"\nCh01. Americas: lower middle income economies (n={len(americas_lower_middle)})")

    # Write to file
    # TODO Uncomment
    write_json('./stu-americas-low_middle.json', americas_lower_middle)
    ...
    # Print slice
    print(f"\n2.1 if-else two income categories (n={len(two_categories)}) = {two_categories[:12]}")
    ...
    # Print slice
    print(f"\n2.2.1 list comp: three income categories (n={len(three_categories)}) = {three_categories[:12]}")
    ...
    # Print slice
    print(f"\n2.2.2 for loop: three income categories (n={len(three_categories)}) = {three_categories[:12]}")

    three_categories = [categorize_economy(country) for country in countries]

    # Print slice
    print(f"\n2.2.3 for loop: three income categories (n={len(three_categories)}) = {three_categories[:12]}")
    ...
    # Print slice
    print(f"\n2.3.2 Arab World (n={len(arab_world)})")
    ...
    sub_saharan_africa = [
       ...
       for group in groups
       for country in countries
       if group['group_code'] == "SSF"
       and group['country_code'] == country['country_code']
   ]

    # Print slice
    print(f"\nCh 02. Sub-Saharan Africa (n={len(sub_saharan_africa)})")
