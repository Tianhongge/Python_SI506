...
def main():
    ...
    west_africa_atlantic = {item[0]: item[1] for item in data} # TODO dict comp
    ...
    inflation_rates = {key: round(val * 100, 2) for key, val in data.items()} # TODO list comp
    ...
    inflation_rates = {year: rate for year, rate in inflation_rates.items() if int(year) > 2009} # TODO list comp

    print(f"\n2.0: US inflation rates 2010-2021 = {inflation_rates}")

    # CHALLENGE 01

    south_asian_economies = {
    ...
    country_income = {ctry['country_code'].upper():
    categorize_economy(ctry) for ctry in countries}
    ...
    china_pop_indicators = {
        'country_code': china['country_code'],
        'country_name': china['country_name'],
        'population_indicators': {
            key: val
            for key, val in china['country_indicators'].items()
            if key.split('.')[1] == 'POP'
        }
    } # TODO Implement list comp
