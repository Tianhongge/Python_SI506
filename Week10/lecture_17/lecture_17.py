...
def main():
    ...
    # BONUS: sort by value
    region_counts = {k: v for k, v in sorted(region_counts.items(), key=lambda x: x[1], reverse=True)} # TODO sort or comment out
    ...
    for site in sites:
        if site['endangered'] == '1':
            if site['region_en'] not in endangered_counts.keys():
                endangered_counts[site['region_en']] = 1
            else:
                endangered_counts[site['region_en']] += 1
    ...
    endangered_counts = dict(sorted(endangered_counts.items(), key=lambda x: x[1], reverse=True)) # TODO sort or comment out
    ...
    for key, val in endangered_counts.items():
        endangered_counts[key] = round(val/count * 100, 2)
    # TODO Implement loop
